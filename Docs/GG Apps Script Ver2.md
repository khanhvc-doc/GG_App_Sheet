# Khung học Google Apps Script — 8 buổi

> Tài liệu này là đặc tả chính thức (single source of truth) của khóa học Google Apps Script 8 buổi, dự án xuyên suốt "Hệ thống quản lý trả nợ môn". Dùng tài liệu này làm căn cứ duy nhất khi soạn bài giảng, bài tập, code mẫu, hoặc chấm bài cho khóa học — không suy đoán thêm ngoài nội dung ở đây.
...
**Điều kiện tiên quyết / tài liệu tự đọc trước buổi 1** (không tính là buổi học riêng):
...
**Thang tiêu chí đánh giá cho mọi buổi** (áp dụng thống nhất, xem cách dùng cụ thể ở mục "Tiêu chí đạt" của từng buổi trong mục 7):
```
...
"Kiểm tra điều kiện" là một bước có logic cụ thể, không phải chỉ là tên bước trong sơ đồ. Điều kiện tối thiểu cần code hóa ở Buổi 4:
...
Hệ thống dùng đúng 6 sheet, tên cố định xuyên suốt tài liệu và code mẫu:
...
| `BAOCAO` | Sheet đích ghi kết quả thống kê (Buổi 3, 7) | tùy báo cáo, xem Buổi 3 / Buổi 7 ở mục 7 |

**Tách rõ trách nhiệm giữa `DANGKY.TrangThai` và `KETQUA.KetQua`** — đây là điểm dễ gây trùng lặp dữ liệu nhất nếu không quy định rõ:
...
**Giới hạn phạm vi của khóa `MSSV + MaMon`** — quy ước rõ ràng cho phạm vi khóa học:
...
Nếu phát triển thành hệ thống thật (nhiều đợt trả nợ, sinh viên có thể đăng ký lại môn đã rớt ở đợt trước), khóa cần nâng cấp thành `MaDangKy` (khóa tự sinh duy nhất) hoặc `MSSV + MaMon + DotTraNo`. Đây là lựa chọn đơn giản hóa có chủ đích cho phạm vi 8 buổi, không phải thiết kế sai.

---

## 4. Lộ trình 8 buổi (tổng quan)
...
**Rủi ro thời gian**: Buổi 4 và Buổi 8 là hai buổi nặng nhất, dễ vỡ tiến độ nếu học viên yếu JS array.
- Buổi 4: tách thành 3 bài nhỏ tuần tự (Validate → GroupBy → Chia nhóm/Lập lịch), chi tiết ở mục 7 — giúp học viên nhìn thấy và debug từng bước thay vì một khối logic lớn.
...
**`doPost()` không thuộc nội dung bắt buộc.** Mục tiêu Buổi 5 là "HTML Form → `google.script.run` → Apps Script → Sheet", và `google.script.run` đã đủ để form gọi hàm ghi dữ liệu phía server. Dạy thêm `doPost()` song song dễ khiến học viên nhầm lẫn giữa hai cơ chế gọi server (điều hướng URL vs RPC qua `google.script.run`) trong khi bài toán không cần đến điều hướng URL. Nếu có học viên khá muốn tìm hiểu thêm (ví dụ để nhận webhook từ bên ngoài Sheet), giới thiệu `doPost()` như nội dung đọc thêm sau Buổi 5, không đưa vào bài tập bắt buộc.
...
Không dạy kiến trúc phần mềm sâu, chỉ giới thiệu **rất nhẹ** từ Buổi 2 để tránh học viên dồn hết logic vào một file `Code.gs` phẳng hàng nghìn dòng đến Buổi 8.

Học viên phân biệt 4 loại hàm và đặt chúng vào các file `.gs` riêng theo module nghiệp vụ:
...
**Kiến trúc code:** Giới thiệu nhẹ cách tách file theo mục 6 (`Sheet.gs`, `SinhVien.gs`...). Từ buổi này, mỗi hàm mới viết ra phải được xếp vào đúng file theo loại (đọc/xử lý/ghi/nghiệp vụ).
...
**Buổi này gồm 3 bài nhỏ tuần tự**, không viết chung thành một khối logic Validate→GroupBy→Chia nhóm→Xếp ngày→Xếp phòng→Ghi→Update liền mạch. Mục đích: học viên chạy và kiểm tra được kết quả từng bước trước khi ghép bước tiếp theo — dễ debug hơn nhiều so với viết một hàm lớn rồi mới chạy thử.
...
```javascript
const SUC_CHUA_PHONG = 30; // giới hạn phạm vi: sức chứa cố định, không tối ưu hóa xếp lịch
...
`doPost()` không nằm trong nội dung bắt buộc buổi này (xem lý do ở mục 5). Nếu còn thời gian hoặc có học viên khá, giới thiệu như đọc thêm, không kiểm tra trong bài tập.
...
**`DataValidation` — dropdown trạng thái trên Sheet:**
```javascript
...
**2 loại trigger** (xem mục 5): `onOpen()/onEdit(e)` là simple trigger; trigger chạy theo giờ (`ScriptApp.newTrigger('capNhatBaoCao').timeBased().everyDays(1).atHour(7).create()`) là installable trigger, cần chạy 1 lần thủ công để cấp quyền.
...
**Phân quyền (khái niệm, không triển khai OAuth/authentication thật):**
...
## 8. Ghi chú cho AI đọc tài liệu này
